...
package projetcc.service;
import java.sql.*;
import projetcc.model.Model;
...
public class Service {
    ...
    public Service(){
        this.url = "jdbc:mysql://localhost:3306/ccpascal";
        this.username = "root";
        try{
    ...
    public boolean ajout_nouveau_etudiant(Model etd){
        try{
            ...
            String reqSQL = "INSERT INTO student value (null,'" + etd.getMatricule() +"','"+ etd.getNom() +"','"+ etd.getFiliere() +"','"+ etd.getNiveau() +"')";
    ...
    public void afficher_liste_etudiant(){
        try{
            Statement req = con.createStatement();
            String reqSQL = "SELECT * FROM student";
    ...
    public void afficher_liste_etudiant_par_filiere(String filiere){
        try{
            Statement req = con.createStatement();
            String reqSQL = "SELECT * FROM student WHERE filiere = '" +filiere+ "' ";
            ResultSet res = req.executeQuery(reqSQL);
            while(res.next()){
    ...
    public void rechercher_etudiant_a_partir_du_matricule(String matricule){
        try{
            Statement req=con.createStatement();
            String reqSQL="SELECT FROM student WHERE matricule = '" +matricule+ "' ";
            ResultSet res=req.executeQuery(reqSQL);
            while(res.next()){
    ...
    public void supprime_matricule(String matricule){
        try{
            Statement req = con.createStatement();
            String sql = "DELETE FROM student WHERE matricule = '" +matricule+ "' ";
    ...
    public void modifier_niveau_etudiant(String matricule,int niveau){
        try{
            Statement req= con.createStatement();
             String sql = "UPDATE student SET niveau = " +niveau+ " WHERE matricule = '" +matricule+ "' ";
             int res=req.executeUpdate(sql);
